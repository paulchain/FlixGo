...
function movie_insert($name_movie,$genre,$release_year,$running_time,$country,$description,$images,$clips,$age,$resolution,$id_cata){
    $sql = "INSERT INTO movie(name_movie,genre,release_year,running_time,country,description,images,clips,age,resolution,id_cata) VALUES(?,?,?,?,?,?,?,?,?,?,?)";
    return pdo_execute($sql, $name_movie,$genre,$release_year,$running_time,$country,$description,$images,$clips,$age,$resolution,$id_cata);
}

function movie_update($id_movie, $name_movie,$genre,$release_year,$running_time,$country,$description,$images,$clips,$age,$resolution,$id_cata){ 
    $sql = "UPDATE movie SET name_movie=?,genre=?,release_year=?,running_time=?,country=?,description=?,images=?,clips=?,age=?,resolution=?,id_cata=? WHERE id_movie=?";
    return pdo_execute($sql, $name_movie,$genre,$release_year,$running_time,$country,$description,$images,$clips,$age,$resolution,$id_cata,$id_movie); 
}
